...
if (isset($_GET['refresh'])) {
    // Clear all ad-related session data to force reshuffling
    ...
    if (isset($_SESSION['original_ads'])) {
        unset($_SESSION['original_ads']);
    }
    if (isset($_SESSION['boosted_posts_shuffled'])) {
        unset($_SESSION['boosted_posts_shuffled']);
    }
...
function getTargetedAds($pdo, $userId, $limit = 20) {
    ...
    // Check if we need to shuffle ads (either first load or refresh requested)
...
// Fetch all active boosted posts globally with shuffling
if (!isset($_SESSION['boosted_posts_shuffled']) || isset($_GET['refresh'])) {
    ...
    $_SESSION['boosted_posts_shuffled'] = $boostedPosts;
} else {
    $boostedPosts = $_SESSION['boosted_posts_shuffled'];
}
...
/* Video overlay for click to pause - Ads Only */
.ad-video-overlay {
    position: absolute;
    top: 0;
    left: 0;
...
<!-- Refresh Button -->
<a href="home.php?refresh=true" class="refresh-btn" id="refreshAdsBtn">🔄 Refresh Ads & Content</a>
...
// Ad Video Controls - Only affects ads
function toggleAdVideoSound(button) {
    const videoContainer = button.closest('.ad-video-container');
    ...
    if (!isMuted && video.paused) {
        video.play().catch(e => console.log('Ad video play prevented:', e));
    }
}

function toggleAdVideoPlayPause(element) {
    const videoContainer = element.classList.contains('ad-video-overlay') 
        ? element.parentElement 
        : element.closest('.ad-video-container');
    const video = videoContainer.querySelector('.ad-video');
    const playPauseBtn = videoContainer.querySelector('.ad-play-pause');
...
// Initialize ad video controls
function initAdVideoControls() {
    document.querySelectorAll('.ad-video-container').forEach(container => {
        const video = container.querySelector('.ad-video');
        const playPauseBtn = container.querySelector('.ad-play-pause');
        const soundBtn = container.querySelector('.ad-sound-toggle');
        
        // Set videos to unmuted by default
        video.muted = false;
        if (soundBtn) {
        ...
        // Auto-play when video is ready (if not paused by user)
        video.addEventListener('loadeddata', () => {
            if (!video.paused) {
                video.play().catch(e => {
                    console.log('Auto-play prevented, muting and retrying:', e);
                    ...
                    video.play().catch(e2 => console.log('Second play attempt prevented:', e2));
                });
            }
        });
    });
...
// Auto-pause ad videos when they're not visible
function initAdVideoObservers() {
    const adVideos = document.querySelectorAll('.ad-video');
    ...
    adVideos.forEach(video => {
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    // Ad video is in view - try to play if not paused
                    if (video.paused) {
                        video.play().catch(e => console.log('Ad video play prevented:', e));
                    }
                } else {
                    // Ad video is out of view - pause it
                    if (!video.paused) {
                        video.pause();
                    }
                }
...
document.addEventListener('DOMContentLoaded', function() {
    initAdVideoControls();
    initAdVideoObservers();
});

// Reinitialize for dynamically loaded ads
setTimeout(() => {
    setInterval(() => {
        const adVideos = document.querySelectorAll('.ad-video');
        adVideos.forEach(video => {
            if (!video.hasAttribute('data-controls-initialized')) {
                video.setAttribute('data-controls-initialized', 'true');
                initAdVideoControls();
                initAdVideoObservers();
            }
        });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (refreshBtn) {
        refreshBtn.addEventListener('click', function(e) {
            ...
            // The actual refresh is handled by the href link
            // This just provides visual feedback
            
            setTimeout(() => {
                this.style.transform = 'rotate(0deg)';
                this.style.opacity = '1';
                this.textContent = '🔄 Refresh Ads & Content';
            }, 1000);
        });
    }
    
    // Check if page was loaded with refresh parameter and show confirmation
    const urlParams = new URLSearchParams(window.location.search);
    if (urlParams.has('refresh')) {
        // Small delay to ensure page is loaded
        setTimeout(() => {
            console.log('Ads and boosted posts have been refreshed and reshuffled!');
