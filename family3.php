...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'add_ad_comment' && isset($_POST['ad_id'])) {
        ...
        try {
            ...
            // Insert comment
            $stmt = $pdo->prepare("
                INSERT INTO ad_comments (ad_id, user_id, content, created_at) 
                VALUES (?, ?, ?, NOW())
            ");
        ...
        ?>
            ...
            <div class="post boosted-post" data-post-id="<?= $boostPost['id'] ?>">
                ...
                <div class="actions">
                    ...
                    <span class="comment-btn" onclick="openPostComments(<?= $boostPost['id'] ?>)">
...
/* Please copy all the CSS from your previous family.php file here */
/* I'm omitting CSS to save space but you should include it all */
</style>
</head>
<body>
...
function attachEventListenersToNewPosts() {
    // Attach like button listeners to new REGULAR posts
    document.querySelectorAll('.like-btn').forEach(button => {
        // Skip ad like buttons (they have different structure)
        if (!button.classList.contains('ad-action-btn') && !button.hasAttribute('data-listener-attached')) {
            button.setAttribute('data-listener-attached', 'true');
            button.addEventListener('click', handleLikeClick);
        }
    });
    
    // Attach like button listeners to new AD posts
    ...
    // Attach ad share button listeners
    document.querySelectorAll('.ad-action-btn.share-btn').forEach(btn => {
        if (!btn.hasAttribute('data-listener-attached')) {
            btn.setAttribute('data-listener-attached', 'true');
            btn.addEventListener('click', function() {
                const adId = this.closest('.ad-post').dataset.adId;
...
async function openPostComments(postId) {
    ...
    try {
        ...
        const response = await fetch('family.php', {
            method: 'POST',
            body: formData
        });
    ...
    <div class="post-comments-container">
        ...
        <section id="comments-section">
            ...
            <div id="comments-list">
                ${comments && comments.length > 0 ? comments.map(comment => generatePostCommentHTML(comment, currentUserId)).join('') : '<div class="no-comments" style="text-align:center; padding:20px; color:#718096;">No comments yet. Be the first to comment!</div>'}
            </div>

            <h3>Add a comment</h3>
            <textarea id="newCommentText" placeholder="Write your comment here..."></textarea>
            <button onclick="addPostComment(${post.id})">Post Comment</button>
...
function generatePostCommentHTML(comment, currentUserId) {
    if (!comment) return '';
...
async function openAdComments(adId) {
    ...
    try {
        ...
        if (data.success) {
            ...
            content.innerHTML = generateAdCommentsHTML(data.ad, data.comments, data.likeCount, data.shareCount, data.userLiked, data.currentUserId);
...
function generateAdCommentsHTML(ad, comments, likeCount, shareCount, userLiked, currentUserId) {
    if (!ad) {
    ...
    <div class="ad-comments-container">
        ...
        <div class="comments-section" id="commentsSection">
            ...
            <!-- Comment Form -->
            <form class="comment-form" onsubmit="submitAdComment(event, ${ad.id})">
            ...
            <div class="comments-list">
                ${comments && comments.length > 0 ? comments.map(comment => generateAdCommentHTML(comment, currentUserId, advertiserId)).join('') : '<div class="no-comments">No comments yet. Be the first to comment!</div>'}
            </div>
        </div>
    </div>
    `;
...
function generateAdCommentHTML(comment, currentUserId, advertiserId) {
    if (!comment) return '';
...
async function submitAdComment(event, adId) {
...
async function likeAdInPopup(adId) {
...
async function shareAdInPopup(adId) {
...
<style>/* Reuse most styles from home.php, but customize for mixed content layout */
body { 
