...
try {
    ...
    $searchStmt = $pdo->prepare("
        ...
        UNION
        (
            SELECT name as term, COUNT(*) as count, 'group' as source
            FROM groups 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'get_suggestions') {
    ...
    try {
        ...
        // Enhanced search in group names with word completion
        $groupStmt = $pdo->prepare("
            SELECT DISTINCT 
                name as term, 
                'group' as type,
                (SELECT COUNT(*) FROM group_members gm WHERE gm.group_id = g.id AND gm.status = 'approved') as member_count,
                g.profile_pic_url as profile_pic
            FROM groups g
            WHERE g.privacy_setting != 'private'
            AND g.name ILIKE ? 
            ORDER BY 
                CASE 
                    WHEN g.name ILIKE ? THEN 1  -- Exact match
                    WHEN g.name ILIKE ? THEN 2  -- Starts with
                    ELSE 3                      -- Contains
                END,
                member_count DESC,
                LENGTH(g.name)
            LIMIT 8
        ");
        $groupStmt->execute([$query . '%', $query, $query . '%']);
        $groupResults = $groupStmt->fetchAll(PDO::FETCH_ASSOC);
        
        // NEW: Enhanced search in post descriptions with word completion
        $descriptionStmt = $pdo->prepare("
            ...
            GROUP BY p.id, p.content, u.username, u.profile_pic_url
            ORDER BY 
                CASE 
                    WHEN p.content ILIKE ? THEN 1  -- Exact match in content
                    WHEN p.content ILIKE ? THEN 2  -- Content starts with
                    ELSE 3                          -- Contains
                END,
                usage_count DESC,
                LENGTH(p.content)
            LIMIT 8
        ");
        $descriptionStmt->execute(['%' . $query . '%', $query, $query . '%']);
        $descriptionResults = $descriptionStmt->fetchAll(PDO::FETCH_ASSOC);
        
        // Process description results to get first 7 words
        foreach ($descriptionResults as &$result) {
            $content = $result['full_content'];
            // Get first 7 words
            $words = preg_split('/\s+/', $content, 8); // Split into max 8 parts
            $firstSevenWords = implode(' ', array_slice($words, 0, 7));
            if (count($words) > 7) {
                $firstSevenWords .= '...';
            }
            $result['term'] = $firstSevenWords;
            $result['full_term'] = $content;
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'get_word_completions') {
    ...
    try {
        ...
        // Get group name completions
        $groupCompletions = $pdo->prepare("
            SELECT 
                name as completion,
                'group' as type,
                (SELECT COUNT(*) FROM group_members gm WHERE gm.group_id = g.id AND gm.status = 'approved') as frequency
            FROM groups g
            WHERE g.privacy_setting != 'private'
            AND g.name ILIKE ? 
              AND LENGTH(g.name) > LENGTH(?)
            ORDER BY frequency DESC, LENGTH(g.name)
            LIMIT 5
        ");
        $groupCompletions->execute([$query . '%', $query]);
        $groupResults = $groupCompletions->fetchAll(PDO::FETCH_ASSOC);
        
        // NEW: Get post description word completions
        $descriptionCompletions = $pdo->prepare("
            SELECT 
                word as completion,
                'description' as type,
                frequency
            FROM (
                SELECT 
                    LOWER(unnest(regexp_split_to_array(content, '\s+'))) as word,
                    COUNT(*) as frequency
                FROM posts 
                WHERE content IS NOT NULL
                  AND LENGTH(content) > 10
                GROUP BY LOWER(unnest(regexp_split_to_array(content, '\s+')))
            ) word_counts
            WHERE word ILIKE ? 
              AND LENGTH(word) > LENGTH(?)
              AND frequency > 1
            ORDER BY frequency DESC, LENGTH(word)
            LIMIT 5
        ");
        $descriptionCompletions->execute([$query . '%', $query]);
        $descriptionResults = $descriptionCompletions->fetchAll(PDO::FETCH_ASSOC);
        
        // Combine all completions
        $allCompletions = array_merge($hashtagResults, $userResults, $headerResults, $groupResults, $descriptionResults);
        
        // Sort by frequency and type priority
        usort($allCompletions, function($a, $b) {
            // Priority: hashtag > user > description > group > header
            $typePriority = [
                'hashtag' => 5,
                'user' => 4,
                'description' => 3,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'get_popular_searches') {
    ...
    try {
        ...
        $popularStmt = $pdo->prepare("
            ...
            UNION
            (
                SELECT name as term, 'group' as type, 
                       (SELECT COUNT(*) FROM group_members gm WHERE gm.group_id = g.id AND gm.status = 'approved') as popularity
                FROM groups g
                WHERE g.privacy_setting != 'private'
                AND g.name ILIKE ? 
                ORDER BY popularity DESC
            ...
            (
                SELECT 
                    CASE 
                        WHEN LENGTH(p.content) > 50 THEN LEFT(p.content, 50) || '...'
                        ELSE p.content
                    END as term, 
                    'description' as type, 
                    COUNT(*) as popularity
            ...
            LIMIT 8
        ");
        $popularStmt->execute([$query . '%', $query . '%', $query . '%', $query . '%', $query . '%']);
...
.trending-badge.group {
    background: linear-gradient(135deg, #4299e1, #3182ce);
}

.trending-badge.hashtag {
    background: linear-gradient(135deg, #7b68ee, #6a5acd);
}

.trending-badge.description {
    background: linear-gradient(135deg, #9f7aea, #805ad5);
}
...
<div class="search-header">
    ...
    <div class="search-container">
        <input type="text" class="search-input" placeholder="Search groups, accounts, hashtags, and videos..." id="searchInput">
    ...
    <?php if (empty($_GET['q'])): ?>
                ...
                <div class="trending-grid">
                    <?php foreach ($trendingHashtags as $index => $hashtag): ?>
                        <div class="trending-item" onclick="searchHashtag('<?= htmlspecialchars($hashtag['tag']) ?>')">
                            ...
                            <div class="trending-badge hashtag">HASHTAG</div>
                        </div>
                    <?php endforeach; ?>
                </div>
            </div>
                    ...
                    <div class="recent-item" onclick="searchTerm('<?= htmlspecialchars($search['term']) ?>')">
                        <div class="recent-text">
                            ...
                            <?php if ($search['source'] === 'hashtag'): ?>
                                <span style="color: #7b68ee; margin-left: 4px;">#</span>
                            <?php elseif ($search['source'] === 'group'): ?>
                                <span style="color: #4299e1; margin-left: 4px;">👥</span>
                            <?php endif; ?>
                        </div>
                        <div style="font-size: 12px; color: #666;">
                            <?= $search['count'] ?> <?= $search['source'] === 'group' ? 'members' : 'posts' ?>
                        </div>
                    </div>
                <?php endforeach; ?>
            </div>
        ...
        <div class="discover-section">
            ...
            <div class="discover-grid">
                ...
                <div class="discover-item" onclick="searchTerm('groups')">
                    <div class="discover-icon">👥</div>
                    <div class="discover-text">Groups</div>
                </div>
            </div>
        </div>
...
class TikTokSearch {
    ...
    displaySuggestions(suggestions, query) {
        ...
        if (suggestions.length === 0) {
            ...
            suggestions.forEach(suggestion => {
                ...
                const count = suggestion.usage_count || suggestion.relevance || suggestion.follower_count || suggestion.member_count || suggestion.frequency;
                const type = suggestion.type;
                const profilePic = suggestion.profile_pic;
                const username = suggestion.username || '';
                const fullTerm = suggestion.full_term || displayText;
                
                html += `
                    <div class="suggestion-item" onclick="tiktokSearch.selectSuggestion('${type}', '${this.escapeHtml(fullTerm)}')">
                        ${profilePic ? 
                            `<img src="${this.escapeHtml(profilePic)}" alt="${this.escapeHtml(displayText)}" class="suggestion-avatar">` :
                            `<div class="suggestion-icon">${this.getSuggestionIcon(type)}</div>`
                        }
                        <div class="suggestion-content">
                            <div class="suggestion-text">${highlightedText}</div>
                            ${type === 'description' && username ? 
                                `<div class="suggestion-description">By ${this.escapeHtml(username)}</div>` : ''}
                            <div class="suggestion-meta">
            ...
            popularHtml += `
                <div class="popular-item" onclick="tiktokSearch.selectSuggestion('${type}', '${this.escapeHtml(displayText)}')">
                    <div>
                        ...
                        ${type === 'description' ? `<div class="popular-description">${this.escapeHtml(displayText)}</div>` : ''}
                    </div>
    ...
    selectSuggestion(type, value) {
        ...
        if (type === 'hashtag') {
            ...
            // For groups, we want to search specifically for groups
            searchQuery = value;
        } else if (type === 'description') {
            // For descriptions, we want to search the full content
            searchQuery = value;
        }
