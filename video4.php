...
// NEW: Content Analysis Service (Simulated AI)
...
// NEW: Real-time Trend Detection Engine
...
// NEW: Advanced User Behavior Model
...
// Enhanced TikTok Algorithm Class with NEW features
class TikTokAdvancedAlgorithm {
    ...
    // NEW: Multi-modal content scoring
    ...
    // NEW: Sound/Effect virality prediction - FIXED
public function predictSoundVirality($post) {
    ...
    // NEW: Advanced cold start for new content
    ...
    // NEW: Real-time engagement velocity
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // USER BEHAVIOR TRACKING HANDLER
    if (isset($_POST['action']) && $_POST['action'] === 'track_behavior' && isset($_POST['post_id'])) {
        ...
        try {
            // MODIFIED: Only count as viewed if watch time is at least 5 seconds
            $effectiveInteractionType = $interactionType;
            if ($interactionType === 'view' && $watchTime < 5) {
                $effectiveInteractionType = 'scroll_past';
            }
            ...
            $stmt = $pdo->prepare("
                ...
                VALUES (?, ?, ?, ?, ?, ?, NOW())
            ");
            $stmt->execute([$userId, $postId, $watchTime, $completionRate, $scrollVelocity, $effectiveInteractionType]);
            
            // Only update session if video was actually watched (5+ seconds)
            if ($watchTime >= 5) {
                $stmt = $pdo->prepare("
            ...
            echo json_encode(['success' => true, 'interaction_type' => $effectiveInteractionType]);
        } catch (Exception $e) {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'negative_feedback' && isset($_POST['post_id'])) {
        ...
        try {
            $stmt = $pdo->prepare("
                ...
                (user_id, post_id, feedback_type, created_at) 
                VALUES (?, ?, ?, NOW())
            ");
            $stmt->execute([$userId, $postId, $feedbackType]);
...
// NEW: Get emerging trends - FIXED with proper error handling
try {
...
// NEW: Get advanced user behavior patterns - FIXED with proper initialization
try {
...
// NEW: Add condition to exclude viewed videos
...
// ENHANCED TIKTOK-STYLE ALGORITHMIC SCORING WITH ALL NEW FEATURES
$scoredPosts = [];
...
foreach ($allPosts as $post) {
    ...
    // NEW: Enhanced scoring with AI features
    ...
    // 1. ENHANCED REAL-TIME ENGAGEMENT METRICS (30% weight)
    $engagementScore = (
        ...
        ($multiModalScores['production_value'] * 2.0) + // NEW
        ($trendScore * 1.5) + // NEW
        ($attentionMatch * 2.0) + // NEW
        ($soundVirality * 1.0) // NEW
    ) * 30 / 100;
    ...
    // 2. ENHANCED USER PERSONALIZATION (25% weight)
    $personalizationScore = 0;
    
    // REMOVED: Follow relationship prioritization
    ...
    // 3. ENHANCED CONTENT QUALITY & CREATOR INFLUENCE (15% weight)
    $qualityScore = (
        ...
        ($multiModalScores['visual_quality'] * 2) + // NEW
        ($multiModalScores['audio_quality'] * 2) + // NEW
        ($multiModalScores['sentiment'] * 1) // NEW
    );
    $score += $qualityScore;
    
    // 4. TIME DECAY WITH USER PATTERNS (10% weight)
    ...
    $score += ($timeDecay * 8) + $timePatternBoost;
    
    // 5. ENHANCED DIVERSITY & DISCOVERY (10% weight)
    $diversityScore = 0;
    
    // Boost new creators and content
    $totalEngagement = $post['likes_count'] + $post['comments_count'] + $post['shares_count'];
    if ($totalEngagement < 50 && !$post['is_following_author']) {
        $diversityScore += 4; // Discovery boost for new content
    }
    
    // Enhanced cold start handling
    if ($isNewUser) {
        $coldStartBoost = $algorithm->enhancedColdStartScoring($post, $userBehavior);
        $diversityScore += $coldStartBoost * 3;
    }
    
    if ($isNewContent) {
        $contentBasedBoost = $algorithm->contentBasedScoring($post, []);
        $diversityScore += $contentBasedBoost * 3;
    }
    
    // Multi-armed bandit exploration bonus
    $explorationBonus = (1 - $post['tiktok_score'] / 100) * 2;
    $diversityScore += $explorationBonus;
    
    $score += $diversityScore;
    
    // 6. MULTI-OBJECTIVE OPTIMIZATION (10% weight)
    $multiObjectiveScore = $algorithm->calculateMultiObjectiveScore($post, $userEngagementScore, $objectives) * 10;
    $score += $multiObjectiveScore;
    
    // 7. NEGATIVE SIGNALS PENALTY (-5% to 0%)
    $negativePenalty = $algorithm->calculateNegativeSignals($post['id']) * 5;
    ...
    $post['score_breakdown'] = [
        ...
        'recency' => round(($timeDecay * 8) + $timePatternBoost, 2),
        'diversity' => round($diversityScore, 2),
        'multi_objective' => round($multiObjectiveScore, 2),
        'negative_penalty' => round($negativePenalty, 2),
    ...
    $post['attention_match'] = round($attentionMatch, 2); // NEW
    $post['sound_virality'] = round($soundVirality, 2); // NEW
    $post['trend_score'] = round($trendScore, 2); // NEW
        ...
        <?php foreach ($posts as $post): ?>
            <div class="reel" data-post-id="<?= $post['id'] ?>">
                ...
                <div class="score-breakdown">
                    ...
                    <div class="score-bar"><div class="score-fill" style="width: <?= ($post['score_breakdown']['engagement'] / 30) * 100 ?>%"></div></div>
                    
                    <div class="score-item">
                        <span class="score-label">Personalization:</span>
                        <span class="score-value"><?= $post['score_breakdown']['personalization'] ?></span>
                    </div>
                    <div class="score-bar"><div class="score-fill" style="width: <?= ($post['score_breakdown']['personalization'] / 25) * 100 ?>%"></div></div>
                    ...
                    <div class="score-bar"><div class="score-fill" style="width: <?= ($post['score_breakdown']['recency'] / 10) * 100 ?>%"></div></div>
                    
                    <div class="score-item">
                        <span class="score-label">Diversity & Discovery:</span>
                        <span class="score-value"><?= $post['score_breakdown']['diversity'] ?></span>
                    </div>
                    <div class="score-bar"><div class="score-fill" style="width: <?= ($post['score_breakdown']['diversity'] / 10) * 100 ?>%"></div></div>
                    
                    <div class="score-item">
                        <span class="score-label">Multi-Objective:</span>
                        <span class="score-value"><?= $post['score_breakdown']['multi_objective'] ?></span>
                    </div>
                    <div class="score-bar"><div class="score-fill" style="width: <?= ($post['score_breakdown']['multi_objective'] / 10) * 100 ?>%"></div></div>
                    
                    <div class="score-item">
                        <span class="score-label">Negative Penalty:</span>
                        <span class="score-value">-<?= $post['score_breakdown']['negative_penalty'] ?></span>
                    </div>
                    <div class="score-bar"><div class="score-fill" style="width: <?= $post['score_breakdown']['negative_penalty'] * 20 ?>%"></div></div>
...
class AdvancedBehaviorTracker {
    ...
    trackScrollPast(postId) {
        ...
        if (!this.watchData.has(postId)) {
            this.sendBehaviorData(postId, 0.1, 'scroll_past');
        }
    }
    ...
    trackVideoWatch(postId, videoIndex, currentTime, duration, eventType) {
        ...
        if (eventType === 'play') {
            ...
            this.sendBehaviorData(postId, 1.0, 'completion');
        } else if (eventType === 'timeupdate') {
            const progress = currentTime / duration;
            
            // MODIFIED: Only count as viewed after 5 seconds
            if (currentTime >= 5 && !postData.countedAsViewed) {
                postData.countedAsViewed = true;
                this.sendBehaviorData(postId, progress, 'view');
            } else if (Math.floor(currentTime) % 5 === 0) {
                this.sendBehaviorData(postId, progress, 'progress');
            }
        }
    }
    
    sendBehaviorData(postId, completionRate, interactionType) {
        const sessionData = this.getSessionData();
        ...
        const postData = this.watchData.get(postId);
        const watchTime = postData ? (Date.now() - postData.startTime) / 1000 : 0;
          ...
          .then(data => {
              if (data.success) {
                  if (interactionType === 'completion') {
                      ...
                      this.showLearningIndicator('📺 Video marked as viewed');
                  }
              }
          })
