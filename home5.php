...
<div class="posts" id="postsContainer">
    ...
    <div class="ad-video-container">
        ...
        <div class="ad-video-controls">
            <button class="ad-sound-toggle" onclick="toggleAdVideoSound(this)" data-muted="true">🔇</button>
        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    // Ad video is in view
                    if (!video.paused) {
                        video.play().catch(e => console.log('Ad video autoplay prevented:', e));
                    }
                } else {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Safely set up mutation observer only if postsContainer exists
    const postsContainer = document.getElementById('postsContainer');
    if (postsContainer) {
        const observer = new MutationObserver(function(mutations) {
            mutations.forEach(function(mutation) {
                if (mutation.addedNodes.length) {
                    // Check if any new ad videos were added
                    mutation.addedNodes.forEach(node => {
                        if (node.nodeType === 1) {
                            // Check if this node or its children contain ad videos
                            const hasAdVideos = node.querySelector && (
                                node.querySelector('.ad-video-container') || 
                                node.classList?.contains('ad-video-container')
                            );
                            if (hasAdVideos) {
                                setTimeout(() => {
                                    initAdVideoControls();
...
// Alternative: Reinitialize on any dynamic content changes (more robust)
setTimeout(() => {
    // Re-check for new ad videos periodically (for dynamic loading)
    setInterval(() => {
        const adVideos = document.querySelectorAll('.ad-video');
        adVideos.forEach(video => {
