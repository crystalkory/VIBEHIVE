...
// NEW: Track profile views and user engagement
if (isset($_GET['profile_viewed'])) {
    $profileViewedId = (int)$_GET['profile_viewed'];
    if ($profileViewedId > 0 && $profileViewedId != $_SESSION['user_id']) {
        $_SESSION['last_profile_viewed'] = $profileViewedId;
        $_SESSION['profile_view_time'] = time();
        
        // Track engagement with this user
        if (!isset($_SESSION['user_engagement'])) {
            $_SESSION['user_engagement'] = [];
        }
        $_SESSION['user_engagement'][$profileViewedId] = 
            ($_SESSION['user_engagement'][$profileViewedId] ?? 0) + 5;
    }
}

// Clear profile view tracking after 2 hours
if (isset($_SESSION['profile_view_time']) && (time() - $_SESSION['profile_view_time']) > 7200) {
    unset($_SESSION['last_profile_viewed']);
    unset($_SESSION['profile_view_time']);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && in_array($_POST['action'], ['follow', 'unfollow']) && isset($_POST['followed_id'])) {
        ...
        if ($followerId && $followedId && $followerId !== $followedId) {
            if ($_POST['action'] === 'follow') {
                ...
                $_SESSION['user_engagement'][$followedId] = ($_SESSION['user_engagement'][$followedId] ?? 0) + 10;
    ...
    if (isset($_POST['action']) && in_array($_POST['action'], ['like', 'unlike']) && isset($_POST['post_id'])) {
        ...
        // Track engagement
        if (!isset($_SESSION['post_engagement'])) {
            $_SESSION['post_engagement'] = [];
        }
        
        // Get post author for engagement tracking
        ...
        if ($postAuthor && $postAuthor != $userId) {
            ...
            $_SESSION['user_engagement'][$postAuthor] = ($_SESSION['user_engagement'][$postAuthor] ?? 0) + 3;
        }
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'load_more_posts') {
        ...
        switch ($stage) {
            case 'mixed':
                $posts = getMixedPosts($pdo, $userId, 10, $offset);
                if (count($posts) < 10) {
                    $nextStage = 'caught_up';
                }
                break;
            ...
            case 'popular':
                $posts = getPopularPosts($pdo, $userId, 10, $offset);
                $hasMore = true;
                break;
        }
...
// NEW: Get users that the current user follows
function getFollowedUsers($pdo, $userId) {
    $stmt = $pdo->prepare("SELECT followed_id FROM follows WHERE follower_id = ?");
    $stmt->execute([$userId]);
    return $stmt->fetchAll(PDO::FETCH_COLUMN, 0);
}

// NEW: Get friends of specific users - FIXED VERSION
function getFriendsOfUsers($pdo, $userIds, $excludeUserId) {
    if (empty($userIds)) return [];
    
    // Create placeholders for the IN clause
    $placeholders = implode(',', array_fill(0, count($userIds), '?'));
    
    // Build parameters: userIds appear twice in the query
    $params = array_merge($userIds, $userIds, [$excludeUserId]);
    
    $query = "
        SELECT DISTINCT 
            CASE 
                WHEN f.user_id IN ($placeholders) THEN f.friend_id 
                ELSE f.user_id 
            END as friend_id
        FROM friends f
        WHERE (f.user_id IN ($placeholders) OR f.friend_id IN ($placeholders))
        AND CASE 
            WHEN f.user_id IN ($placeholders) THEN f.friend_id 
            ELSE f.user_id 
        END != ?
    ...
    try {
        ...
        error_log("Error in getFriendsOfUsers: " . $e->getMessage());
        return [];
    }
}

// NEW: Get posts from friends of users that the current user follows (last 3 days) - FIXED
function getFriendsOfFollowedPosts($pdo, $userId, $limit = 10, $offset = 0) {
    $followedUsers = getFollowedUsers($pdo, $userId);
    
    if (empty($followedUsers)) {
        return [];
    }
    
    $friendsOfFollowed = getFriendsOfUsers($pdo, $followedUsers, $userId);
    
    if (empty($friendsOfFollowed)) {
        return [];
    }
    
    $placeholders = implode(',', array_fill(0, count($friendsOfFollowed), '?'));
    
    // Build parameters correctly
    $params = $friendsOfFollowed;
    $params[] = $userId;
    $params[] = $limit;
    $params[] = $offset;
    ...
    try {
        ...
        error_log("Error in getFriendsOfFollowedPosts: " . $e->getMessage());
        return [];
    }
}

// NEW: Get posts from profile viewed user
function getProfileViewedPosts($pdo, $userId, $limit = 10, $offset = 0) {
    if (!isset($_SESSION['last_profile_viewed']) || $_SESSION['last_profile_viewed'] == $userId) {
        return [];
    }
    
    $profileUserId = $_SESSION['last_profile_viewed'];
    ...
    $query = "
        ...
        AND p.group_id IS NULL
        AND p.created_at >= NOW() - INTERVAL '3 days'
        AND (p.privacy_setting = 'public' OR p.privacy_setting = 'friends' OR 
             (p.privacy_setting = 'friends' AND EXISTS (
    ...
    $stmt = $pdo->prepare($query);
    $stmt->execute([$profileUserId, $userId, $userId, $limit, $offset]);
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}

// NEW: Get random posts from last 7 days
function getRandomRecentPosts($pdo, $userId, $limit = 10, $offset = 0) {
    $query = "
        SELECT p.*, u.username, u.profile_pic_url, u.is_business_account, u.id AS author_id
        FROM posts p
        JOIN users u ON p.user_id = u.id
        ...
        AND (p.privacy_setting = 'public' OR 
             (p.privacy_setting = 'friends' AND EXISTS (
                 SELECT 1 FROM friends f WHERE 
                 ((f.user_id = ? AND f.friend_id = p.user_id) OR 
                  (f.friend_id = ? AND f.user_id = p.user_id)) 
...
// NEW: Get popular posts from last 6 days - FIXED
function getPopularPosts($pdo, $userId, $limit = 10, $offset = 0) {
    $query = "
        SELECT p.*, u.username, u.profile_pic_url, u.is_business_account, u.id AS author_id,
...
// NEW: Get posts from users with high engagement
function getEngagedUsersPosts($pdo, $userId, $limit = 10, $offset = 0) {
    if (!isset($_SESSION['user_engagement']) || empty($_SESSION['user_engagement'])) {
        return [];
    }
    
    // Get top 5 engaged users
    arsort($_SESSION['user_engagement']);
    $engagedUsers = array_slice(array_keys($_SESSION['user_engagement']), 0, 5);
    
    if (empty($engagedUsers)) {
        return [];
    }
    
    $placeholders = implode(',', array_fill(0, count($engagedUsers), '?'));
    $params = array_merge($engagedUsers, [$limit, $offset]);
...
// NEW: Apply post mixing logic with user interval
function applyPostMixingLogic($posts, $limit) {
    if (empty($posts)) return [];
    
    // Track last positions of users
    $userLastPosition = [];
    $mixedPosts = [];
    $postQueue = $posts;
    ...
    while (count($mixedPosts) < $limit && count($postQueue) > 0 && $attempts < $maxAttempts) {
        $attempts++;
        
        // Try to find a post that satisfies the interval condition
        $found = false;
        foreach ($postQueue as $key => $post) {
            $authorId = $post['user_id'];
            $currentPosition = count($mixedPosts);
            
            // Check if this user has posted recently (within 2 posts)
            if (!isset($userLastPosition[$authorId]) || 
                ($currentPosition - $userLastPosition[$authorId] >= 2)) {
                
                // Add this post
                $mixedPosts[] = $post;
                $userLastPosition[$authorId] = $currentPosition;
                unset($postQueue[$key]);
                $found = true;
                break;
            }
        ...
        // If no post satisfies the condition, take the next one
        if (!$found && !empty($postQueue)) {
            $post = array_shift($postQueue);
            $mixedPosts[] = $post;
            $authorId = $post['user_id'];
            $userLastPosition[$authorId] = count($mixedPosts) - 1;
        }
        
        // Reset array keys
        $postQueue = array_values($postQueue);
    }
    
    // Shuffle the final array for better mix
    shuffle($mixedPosts);
    
    return array_slice($mixedPosts, 0, $limit);
}

// NEW: Main function to get mixed posts with all criteria - IMPROVED
function getMixedPosts($pdo, $userId, $limit = 10, $offset = 0) {
    $allPosts = [];
    
    // Ensure we get integers for limits
    $limit = (int)$limit;
    $offset = (int)$offset;
    
    // Calculate distribution with proper rounding
    $friendsLimit = max(1, (int)ceil($limit * 0.4));
    $profileLimit = max(1, (int)ceil($limit * 0.3));
    $randomLimit = max(1, (int)ceil($limit * 0.2));
    $popularLimit = max(1, (int)ceil($limit * 0.1));
    $engagedLimit = max(1, (int)ceil($limit * 0.1));
    
    // 1. Get posts from friends of followed users (last 3 days)
    $friendsOfFollowedPosts = getFriendsOfFollowedPosts($pdo, $userId, $friendsLimit, $offset);
    $allPosts = array_merge($allPosts, $friendsOfFollowedPosts);
    
    // 2. Get posts from profile viewed user (if any)
    if (isset($_SESSION['last_profile_viewed']) && $_SESSION['last_profile_viewed'] != $userId) {
        $profilePosts = getProfileViewedPosts($pdo, $userId, $profileLimit, $offset);
        $allPosts = array_merge($allPosts, $profilePosts);
    }
    
    // 3. Get random posts from last 7 days
    $randomPosts = getRandomRecentPosts($pdo, $userId, $randomLimit, $offset);
    $allPosts = array_merge($allPosts, $randomPosts);
    
    // 4. Get popular posts from last 6 days
    $popularPosts = getPopularPosts($pdo, $userId, $popularLimit, $offset);
    $allPosts = array_merge($allPosts, $popularPosts);
    
    // 5. Get posts from users with high engagement
    $engagedPosts = getEngagedUsersPosts($pdo, $userId, $engagedLimit, $offset);
    $allPosts = array_merge($allPosts, $engagedPosts);
    
    // Apply post mixing logic with user interval
    $mixedPosts = applyPostMixingLogic($allPosts, $limit);
    
    return enhancePosts($pdo, $userId, $mixedPosts);
}

// Enhanced posts with additional data
...
// Get initial posts using the new mixed algorithm
$initialPosts = getMixedPosts($pdo, $userId, 15, 0);
...
// The rest of your HTML, CSS, and JavaScript code remains the same...
// [Include your existing HTML, CSS, and JavaScript here]
?>
