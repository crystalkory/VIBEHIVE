...
class ProfessionalVideoEditor {
    ...
    // Add audio to video - FIXED VERSION
    ...
    // Add text overlay - FIXED VERSION with draggable support
    ...
    // Add watermark to video with bigger text and final frame
    public function addWatermark($videoPath, $outputPath, $watermarkText = "Tech Titans") {
        // Get video duration
        $videoInfo = $this->getVideoInfo($videoPath);
        $duration = $videoInfo['duration'];
        
        // Create a temporary file for the final frame
        $finalFramePath = $this->tempDir . 'final_frame_' . uniqid() . '.png';
        
        // Create a blue background image with white text
        $command = "{$this->ffmpegPath} -f lavfi -i color=c=blue:s=1920x1080:d=1 -vf \"drawtext=text='{$watermarkText}':fontsize=72:fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2:box=1:boxcolor=black@0.7:boxborderw=10\" -frames:v 1 \"{$finalFramePath}\"";
        exec($command, $output, $returnCode);
        ...
        // Create a temporary file for the video with watermark
        $tempVideoPath = $this->tempDir . 'temp_watermark_' . uniqid() . '.mp4';
        
        // Add watermark to the video with bigger text
        ...
        $command = "{$this->ffmpegPath} -i \"{$videoPath}\" -vf \"{$filter}\" -c:a copy \"{$tempVideoPath}\"";
        exec($command, $output, $returnCode);
        ...
        // Concatenate the watermarked video with the final frame
        $concatListPath = $this->tempDir . 'concat_list_' . uniqid() . '.txt';
        $concatContent = "file '" . realpath($tempVideoPath) . "'\nfile '" . realpath($finalFramePath) . "'\n";
        file_put_contents($concatListPath, $concatContent);
        
        $command = "{$this->ffmpegPath} -f concat -safe 0 -i \"{$concatListPath}\" -c copy \"{$outputPath}\"";
        exec($command, $output, $returnCode);
        
        // Clean up temporary files
        unlink($tempVideoPath);
        unlink($finalFramePath);
        unlink($concatListPath);
        
        return $returnCode === 0;
    }
    
    // Apply filter to video - EXTENDED WITH SNAPCHAT-LIKE FILTERS
    ...
    // Create video from images WITH WATERMARK AND FINAL FRAME - FIXED VERSION
    public function createVideoFromImages($images, $outputPath, $fps = 24, $duration = 5) {
        ...
        $tempVideoPath = $this->tempDir . 'temp_video_' . uniqid() . '.mp4';
        $command = "{$this->ffmpegPath} -f concat -safe 0 -i \"{$tempListFile}\" -r {$fps} -pix_fmt yuv420p \"{$tempVideoPath}\"";
        exec($command, $output, $returnCode);
        
        unlink($tempListFile);
        
        if ($returnCode === 0) {
            // Add watermark and final frame to the created video
            $result = $this->addWatermark($tempVideoPath, $outputPath);
            // Clean up temp file
    ...
    // Export final video WITH WATERMARK AND FINAL FRAME
    public function exportVideo($videoPath, $outputPath, $quality = 'high') {
        $qualitySettings = [
            'low' => '-crf 28 -preset fast -vf scale=640:360',
            'medium' => '-crf 23 -preset medium -vf scale=1280:720',
            'high' => '-crf 18 -preset slow -vf scale=1920:1080',
            '4k' => '-crf 18 -preset slow -vf scale=3840:2160'
        ];
        
        $settings = $qualitySettings[$quality] ?? $qualitySettings['high'];
        
        // First export with quality settings
        $tempVideoPath = $this->tempDir . 'temp_export_' . uniqid() . '.mp4';
        $command = "{$this->ffmpegPath} -i \"{$videoPath}\" {$settings} \"{$tempVideoPath}\"";
        exec($command, $output, $returnCode);
        
        if ($returnCode === 0) {
            // Add watermark and final frame to exported video
            $result = $this->addWatermark($tempVideoPath, $outputPath);
            // Clean up temp file
            if (file_exists($tempVideoPath)) {
                unlink($tempVideoPath);
        ...
        switch ($_POST['action']) {
                ...
                if (!empty($_FILES['media']['name'])) {
                    ...
                    if (move_uploaded_file($file['tmp_name'], $filePath)) {
                        ...
                        if ($mediaType === 'video') {
                            ...
                            // For images, we'll create a video with watermark
            ...
            case 'upload_multiple_images':
                if (!empty($_FILES['images']['name'][0])) {
                    ...
                    if (!empty($imagePaths)) {
                        // Create video from images (watermark will be added automatically)
                        ...
                        if ($result) {
                            // Extract frames from the created video
                            $frameDir = $uploadDir . 'frames/' . pathinfo($outputFilename, PATHINFO_FILENAME);
                            $frameResult = $videoEditor->extractFrames($outputPath, $frameDir);
                            $videoInfo = $videoEditor->getVideoInfo($outputPath);
            ...
            case 'trim_video':
                if (!empty($_SESSION['current_media']) && $_SESSION['current_media']['type'] === 'video') {
                    ...
                    if ($result) {
                        // Add watermark to trimmed video
                        $watermarkedPath = $uploadDir . 'watermarked_' . $outputFilename;
                        if ($videoEditor->addWatermark($outputPath, $watermarkedPath)) {
                            $outputPath = $watermarkedPath;
                            $outputFilename = 'watermarked_' . $outputFilename;
                        }
            ...
            case 'split_video':
                if (!empty($_SESSION['current_media']) && $_SESSION['current_media']['type'] === 'video') {
                    ...
                    if ($result) {
                        // Add watermark to both split videos
                        $watermarkedPath1 = $uploadDir . 'watermarked_' . $outputFilename1;
                        $watermarkedPath2 = $uploadDir . 'watermarked_' . $outputFilename2;
                        
                        if ($videoEditor->addWatermark($outputPath1, $watermarkedPath1) && 
                            $videoEditor->addWatermark($outputPath2, $watermarkedPath2)) {
                            $outputPath1 = $watermarkedPath1;
                            $outputPath2 = $watermarkedPath2;
                            $outputFilename1 = 'watermarked_' . $outputFilename1;
                            $outputFilename2 = 'watermarked_' . $outputFilename2;
                        }
            ...
            case 'delete_frame':
                if (!empty($_SESSION['current_media']) && $_SESSION['current_media']['type'] === 'video') {
                    ...
                    if (isset($frames[$frameIndex])) {
                        ...
                        $_SESSION['current_media']['frames'] = $frames;
                        $_SESSION['current_media']['frame_count'] = count($frames);
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .video-clip { background: var(--primary); }
        .audio-clip { background: #4CAF50; }
        .text-clip { background: #FF9800; }
        .effect-clip { background: #9C27B0; }
...
<body>
    <div class="editor-container">
        ...
        <div class="editor-header">
            <div class="logo">
                <i class="fas fa-video" style="color: var(--primary); font-size: 24px;"></i>
    ...
    <script>
        ...
        function uploadMultipleImages() {
            ...
            showToast('Creating video from images...', 'info');
            ...
            frames.forEach((frame, index) => {
                ...
                frameElement.innerHTML = `
                    ...
                    <div class="frame-actions">
                        <button class="frame-btn" onclick="deleteFrame(${index})" title="Delete Frame">
        ...
        function deleteFrame(frameIndex) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    displayFrames(data.frames, videoDuration);
                } else {
        ...
        function deleteSelectedFrames() {
            ...
            if (!confirm(`Are you sure you want to delete ${selectedFrames.size} frame(s)?`)) {
                return;
            }

            // Delete frames one by one (in reverse order to maintain indices)
            const framesToDelete = Array.from(selectedFrames).sort((a, b) => b - a);
            let deleteCount = 0;

            function deleteNextFrame() {
                if (deleteCount >= framesToDelete.length) {
                    showToast(`Successfully deleted ${deleteCount} frame(s)!`, 'success');
                    closeModal('frameManagerModal');
                    return;
                }

                const frameIndex = framesToDelete[deleteCount];
                const formData = new FormData();
                formData.append('ajax', 'true');
                formData.append('action', 'delete_frame');
                formData.append('frame_index', frameIndex);
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        showToast('Failed to delete some frames', 'error');
                    }
                })
                .catch(error => {
                    console.error('Error:', error);
            ...
            showToast(`Deleting ${framesToDelete.length} frame(s)...`, 'info');
            deleteNextFrame();
        }
        ...
        function duplicateFrame() {
            ...
            showToast('Duplicating frame...', 'info');
        ...
        function splitVideo() {
            ...
            showToast('Splitting video...', 'info');
        ...
        function adjustSpeed() {
            ...
            showToast('Adjusting video speed...', 'info');
        ...
        function addAudioToVideo() {
            ...
            showToast('Adding audio to video...', 'info');
        ...
        function addTextToVideo() {
            ...
            showToast('Adding text to video...', 'info');
        ...
        function applyFilter(filter) {
            ...
            showToast(`Applying ${filter} filter...`, 'info');
        ...
        function reverseVideo() {
            ...
            showToast('Reversing video...', 'info');
        ...
        function stabilizeVideo() {
            ...
            showToast('Stabilizing video...', 'info');
