...
// Helpers for comment and like counts and user liked check
...
// Fetch photo posts of logged-in user
$stmt = $pdo->prepare("
    SELECT p.*, u.username, u.profile_pic_url
    FROM posts p
    JOIN users u ON p.user_id = u.id
    WHERE p.user_id = :user_id AND p.post_type = 'photo'
...
<?php if (!$posts): ?>
    <p>You have no photos posted yet.</p>
<?php else: ?>
    ...
    <div class="post" data-post-id="<?= $post['id'] ?>">
        ...
        <?php endif; ?>
        <?php
        $photos = explode(',', $post['media_url']);
        foreach ($photos as $photo):
        ?>
            <img src="<?= htmlspecialchars($photo) ?>" alt="Photo Post" class="post-image" />
        <?php endforeach; ?>
        <div class="actions">
...
<script>
// Show More / Show Less toggle for post content
