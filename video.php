            ...
            <div class="video-overlay">
                ...
                <div class="action-buttons">
                    ...
                    <div class="action-button comment-btn" 
                         onclick="openCommentModal(<?= $post['id'] ?>, 'post')">
        ...
        <div class="reel ad-reel" data-ad-id="<?= $ad['id'] ?>" data-index="<?= $index ?>">
            ...
            <div class="video-overlay">
                ...
                <div class="action-buttons">
                    ...
                    <div class="action-button comment-btn" 
                         onclick="openCommentModal(<?= $ad['id'] ?>, 'ad')">
                        <div class="action-icon">💬</div>
...
/* Ensure the avatar container also has proper pointer events */
.user-info {
    pointer-events: auto;
    z-index: 15;
}
...
function openCommentModal(itemId, type) {
    ...
    content.innerHTML = `
        <div style="padding: 40px; text-align: center;">
            <div style="border: 4px solid #f3f3f3; border-top: 4px solid #7b68ee; border-radius: 50%; width: 40px; height: 40px; animation: spin 1s linear infinite; margin: 0 auto;"></div>
...
class TikTokReelsScroll {
    ...
    initEventListeners() {
        ...
        // Comment buttons - ADDED EVENT LISTENERS FOR BOTH POST AND AD
        document.querySelectorAll('.comment-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                if (reel.classList.contains('ad-reel')) {
                    const adId = reel.getAttribute('data-ad-id');
        ...
        document.querySelectorAll('.share-btn').forEach(btn => {
            btn.addEventListener('click', this.shareItem.bind(this));
        });
    ...
    shareItem(e) {
        ...
        const reel = e.currentTarget.closest('.reel');
        if (reel.classList.contains('ad-reel')) {
            const adId = reel.getAttribute('data-ad-id');
            this.shareAd(adId);
        } else {
    ...
    sharePost(postId) {
        const postUrl = `${window.location.origin}/comment.php?post_id=${postId}`;
        ...
        if (navigator.share) {
            navigator.share({
                ...
                this.copyToClipboard(postUrl, postId, false);
            });
        } else {
            this.copyToClipboard(postUrl, postId, false);
        }
    ...
    attachEventListenersToNewVideos() {
        ...
        // Comment buttons for new posts and ads
        document.querySelectorAll('.comment-btn:not([data-listener-attached])').forEach(btn => {
            btn.setAttribute('data-listener-attached', 'true');
            btn.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
        ...
        // Share buttons for new posts and ads
        document.querySelectorAll('.share-btn:not([data-listener-attached])').forEach(btn => {
            btn.setAttribute('data-listener-attached', 'true');
            btn.addEventListener('click', this.shareItem.bind(this));
        });
    }
