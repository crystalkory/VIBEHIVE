<?php
// group_editor_video.php - PROFESSIONAL GROUP VIDEO EDITOR
session_start();
...
// Get group ID from session or URL
$groupId = isset($_SESSION['current_group_id']) ? $_SESSION['current_group_id'] : (isset($_GET['group_id']) ? (int)$_GET['group_id'] : null);
if (!$groupId) {
...
function saveGroupEditedVideoToDatabase($pdo, $userId, $groupId, $videoPath, $filename) {
    try {
        $stmt = $pdo->prepare("
            INSERT INTO group_edited_videos (user_id, group_id, video_path, filename, created_at) 
            VALUES (?, ?, ?, ?, NOW())
        ");
        $stmt->execute([$userId, $groupId, $videoPath, $filename]);
        return $pdo->lastInsertId();
    } catch (PDOException $e) {
        error_log("Error saving group edited video: " . $e->getMessage());
        return false;
    }
}
...
if (!isset($_SESSION['group_video_info_cache'])) {
    $_SESSION['group_video_info_cache'] = [];
}

// Video Editor Class (Same as editor_video.php but with group context)
class GroupVideoEditor {
    private $uploadDir;
    private $tempDir;
    private $ffmpegPath;
    
    public function __construct() {
        $this->uploadDir = __DIR__ . '/group_video_uploads/';
        $this->tempDir = __DIR__ . '/group_video_temp/';
    ...
    public function getVideoInfo($videoPath) {
        // Check cache first
        if (isset($_SESSION['group_video_info_cache'][$videoPath])) {
            return $_SESSION['group_video_info_cache'][$videoPath];
        }
        ...
        // Cache the result
        $_SESSION['group_video_info_cache'][$videoPath] = $info;
    ...
    public function addWatermark($videoPath, $outputPath, $watermarkText = "Group Video") {
        // Get video duration
    ...
    public function exportVideo($videoPath, $outputPath, $quality = 'high') {
        ...
        $watermarkFilter = "drawtext=text='Group Video':x=w-text_w-10:y=h-text_h-10:fontsize=36:fontcolor=white@0.8:box=1:boxcolor=black@0.5:boxborderw=8";
...
$videoEditor = new GroupVideoEditor();
$uploadDir = __DIR__ . '/group_video_uploads/';

// Initialize session for undo/redo functionality
if (!isset($_SESSION['group_editor_history'])) {
    $_SESSION['group_editor_history'] = [];
    $_SESSION['group_editor_history_index'] = -1;
}

// Function to save state to history
function saveToGroupHistory($mediaData) {
    $_SESSION['group_editor_history'][] = $mediaData;
    $_SESSION['group_editor_history_index'] = count($_SESSION['group_editor_history']) - 1;
    
    // Limit history size to prevent memory issues
    if (count($_SESSION['group_editor_history']) > 20) {
        array_shift($_SESSION['group_editor_history']);
        $_SESSION['group_editor_history_index']--;
    }
}
        ...
        switch ($_POST['action']) {
            case 'upload_media':
                if (!empty($_FILES['media']['name'])) {
                    ...
                    $filename = uniqid('group_video_') . '.' . $fileExt;
                    ...
                    if (move_uploaded_file($file['tmp_name'], $filePath)) {
                        ...
                        if ($mediaType === 'video') {
                            ...
                            $mediaData = [
                                'type' => 'video',
                                'path' => $filePath,
                                'url' => 'group_video_uploads/' . $filename,
                                'filename' => $filename,
                            ...
                            $_SESSION['current_group_media'] = $mediaData;
                            saveToGroupHistory($mediaData);
                        } else {
                            // For images, create a video
                            $outputFilename = 'group_image_video_' . uniqid() . '.mp4';
                            ...
                            if ($videoEditor->createVideoFromImages([$filePath], $outputPath, 1, 5)) {
                                ...
                                $mediaData = [
                                    'type' => 'video',
                                    'path' => $outputPath,
                                    'url' => 'group_video_uploads/' . $outputFilename,
                                    ...
                                    'resolution' => $videoInfo['resolution'],
                                    'fps' => $videoInfo['fps'],
                                    'source_image' => 'group_video_uploads/' . $filename
                                ];
                        ...
                        $response = [
                            'success' => true,
                            'mediaType' => $mediaType,
                            'url' => $_SESSION['current_group_media']['url'],
                            'frames' => $_SESSION['current_group_media']['frames'] ?? [],
                            'duration' => $_SESSION['current_group_media']['duration'] ?? 0
                        ];
                    }
                }
                break;
                ...
                if (!empty($_FILES['images']['name'][0])) {
                    ...
                    foreach ($_FILES['images']['tmp_name'] as $key => $tmpName) {
                        if ($_FILES['images']['error'][$key] === UPLOAD_ERR_OK) {
                            $filename = uniqid('group_image_') . '_' . $_FILES['images']['name'][$key];
                            $filePath = $uploadDir . $filename;
                            
                            if (move_uploaded_file($tmpName, $filePath)) {
                                $uploadedImages[] = 'group_video_uploads/' . $filename;
                                $imagePaths[] = $filePath;
                            }
                        }
                    }
                    ...
                    if (!empty($imagePaths)) {
                        // Create video from images
                        $outputFilename = 'group_slideshow_' . uniqid() . '.mp4';
                        $outputPath = $uploadDir . $outputFilename;
                        ...
                        if ($result) {
                            ...
                            $response = [
                                ...
                                'url' => 'group_video_uploads/' . $outputFilename,
                                'frames' => $_SESSION['current_group_media']['frames'],
                                'duration' => $_SESSION['current_group_media']['duration']
                            ];
                        } else {
            ...
            case 'trim_video':
                if (!empty($_SESSION['current_group_media']) && $_SESSION['current_group_media']['type'] === 'video') {
                    $videoPath = $_SESSION['current_group_media']['path'];
                    $outputFilename = 'group_trimmed_' . uniqid() . '.mp4';
                    $outputPath = $uploadDir . $outputFilename;
                    
                    $startTime = floatval($_POST['start_time'] ?? 0);
                    $duration = floatval($_POST['duration'] ?? $_SESSION['current_group_media']['duration']);
                    ...
                    if ($result) {
                        ...
                        $mediaData = [
                            ...
                            'frame_count' => $frameResult['frame_count'] ?? 0,
                            'duration' => $videoInfo['duration'],
                            'resolution' => $_SESSION['current_group_media']['resolution'],
                            'fps' => $_SESSION['current_group_media']['fps']
                        ];
            ...
            case 'split_video':
                if (!empty($_SESSION['current_group_media']) && $_SESSION['current_group_media']['type'] === 'video') {
                    $videoPath = $_SESSION['current_group_media']['path'];
                    $splitTime = floatval($_POST['split_time'] ?? $_SESSION['current_group_media']['duration'] / 2);
                    
                    $outputFilename1 = 'group_split1_' . uniqid() . '.mp4';
                    $outputFilename2 = 'group_split2_' . uniqid() . '.mp4';
                    ...
                    if ($result) {
                        ...
                        $_SESSION['group_split_videos'] = [
                            [
                                'url' => 'group_video_uploads/' . $outputFilename1,
                                'duration' => $splitTime
                            ],
                            [
                                'url' => 'group_video_uploads/' . $outputFilename2,
                                'duration' => $_SESSION['current_group_media']['duration'] - $splitTime
                            ]
                        ];
                        
                        $response = [
                            'success' => true,
                            'videos' => $_SESSION['group_split_videos']
                        ];
                    }
                }
                break;
            ...
            case 'adjust_speed':
                if (!empty($_SESSION['current_group_media']) && $_SESSION['current_group_media']['type'] === 'video') {
                    $videoPath = $_SESSION['current_group_media']['path'];
                    $outputFilename = 'group_speed_' . uniqid() . '.mp4';
                    $outputPath = $uploadDir . $outputFilename;
            ...
            case 'add_text_overlay':
                if (!empty($_SESSION['current_group_media']) && $_SESSION['current_group_media']['type'] === 'video') {
                    $videoPath = $_SESSION['current_group_media']['path'];
                    $outputFilename = 'group_text_' . uniqid() . '.mp4';
                    $outputPath = $uploadDir . $outputFilename;
                    
                    $textOptions = [
                        'text' => $_POST['text'] ?? 'Group Text',
                        'x' => intval($_POST['x'] ?? 10),
                        'y' => intval($_POST['y'] ?? 10),
            ...
            case 'apply_filter':
                if (!empty($_SESSION['current_group_media']) && $_SESSION['current_group_media']['type'] === 'video') {
                    $videoPath = $_SESSION['current_group_media']['path'];
                    $outputFilename = 'group_filtered_' . uniqid() . '.mp4';
                    $outputPath = $uploadDir . $outputFilename;
            ...
            case 'remove_audio':
                if (!empty($_SESSION['current_group_media']) && $_SESSION['current_group_media']['type'] === 'video') {
                    $videoPath = $_SESSION['current_group_media']['path'];
                    $outputFilename = 'group_no_audio_' . uniqid() . '.mp4';
                    $outputPath = $uploadDir . $outputFilename;
            ...
            case 'add_audio':
                if (!empty($_SESSION['current_group_media']) && $_SESSION['current_group_media']['type'] === 'video' && !empty($_FILES['audio']['name'])) {
                    $videoPath = $_SESSION['current_group_media']['path'];
                    $audioFile = $_FILES['audio'];
                    $audioFilename = uniqid('group_audio_') . '.' . pathinfo($audioFile['name'], PATHINFO_EXTENSION);
                    $audioPath = $uploadDir . $audioFilename;
                    
                    if (move_uploaded_file($audioFile['tmp_name'], $audioPath)) {
                        $outputFilename = 'group_with_audio_' . uniqid() . '.mp4';
                        $outputPath = $uploadDir . $outputFilename;
            ...
            case 'export_video':
                if (!empty($_SESSION['current_group_media']) && $_SESSION['current_group_media']['type'] === 'video') {
                    $videoPath = $_SESSION['current_group_media']['path'];
                    $outputFilename = 'group_export_' . uniqid() . '.mp4';
                    $outputPath = $uploadDir . $outputFilename;
            ...
            case 'delete_frame':
                if (!empty($_SESSION['current_group_media']) && $_SESSION['current_group_media']['type'] === 'video') {
                    ...
                    if (isset($frames[$frameIndex])) {
                        ...
                        $originalPath = $_SESSION['current_group_media']['path'];
                        $outputFilename = 'group_reconstructed_' . uniqid() . '.mp4';
                        $outputPath = $uploadDir . $outputFilename;
            ...
            case 'duplicate_frame':
                if (!empty($_SESSION['current_group_media']) && $_SESSION['current_group_media']['type'] === 'video') {
                    $videoPath = $_SESSION['current_group_media']['path'];
                    $outputFilename = 'group_duplicated_' . uniqid() . '.mp4';
                    $outputPath = $uploadDir . $outputFilename;
            ...
            case 'undo':
                if (!empty($_SESSION['group_editor_history']) && $_SESSION['group_editor_history_index'] > 0) {
                    $_SESSION['group_editor_history_index']--;
                    $_SESSION['current_group_media'] = $_SESSION['group_editor_history'][$_SESSION['group_editor_history_index']];
            ...
            case 'redo':
                if (!empty($_SESSION['group_editor_history']) && $_SESSION['group_editor_history_index'] < count($_SESSION['group_editor_history']) - 1) {
                    $_SESSION['group_editor_history_index']++;
                    $_SESSION['current_group_media'] = $_SESSION['group_editor_history'][$_SESSION['group_editor_history_index']];
            ...
            case 'save_and_redirect':
                if (!empty($_SESSION['current_group_media']) && $_SESSION['current_group_media']['type'] === 'video') {
                    $videoPath = $_SESSION['current_group_media']['path'];
                    $filename = $_SESSION['current_group_media']['filename'];
                    
                    // Apply watermark before saving
                    $watermarkedFilename = 'group_final_' . uniqid() . '.mp4';
                    $watermarkedPath = $uploadDir . $watermarkedFilename;
                    
                    if ($videoEditor->applyFinalWatermark($videoPath, $watermarkedPath)) {
                        $url = 'group_video_uploads/' . $watermarkedFilename;
                        
                        // Save to database
                        $videoId = saveGroupEditedVideoToDatabase($pdo, $_SESSION['user_id'], $groupId, $url, $watermarkedFilename);
                        
                        if ($videoId) {
                            // Store in session for group_post.php
                            $_SESSION['group_edited_video'] = [
                                'type' => 'video',
                                'path' => $url,
                            ...
                            $response = [
                                'success' => true,
                                'redirect_url' => 'group_post.php?group_id=' . $groupId
                            ];
                        } else {
            ...
            case 'reverse_video':
                if (!empty($_SESSION['current_group_media']) && $_SESSION['current_group_media']['type'] === 'video') {
                    $videoPath = $_SESSION['current_group_media']['path'];
                    $outputFilename = 'group_reversed_' . uniqid() . '.mp4';
                    $outputPath = $uploadDir . $outputFilename;
            ...
            case 'extract_audio':
                if (!empty($_SESSION['current_group_media']) && $_SESSION['current_group_media']['type'] === 'video') {
                    $videoPath = $_SESSION['current_group_media']['path'];
                    $outputFilename = 'group_audio_' . uniqid() . '.mp3';
                    $outputPath = $uploadDir . $outputFilename;
            ...
            case 'stabilize_video':
                if (!empty($_SESSION['current_group_media']) && $_SESSION['current_group_media']['type'] === 'video') {
                    $videoPath = $_SESSION['current_group_media']['path'];
                    $outputFilename = 'group_stabilized_' . uniqid() . '.mp4';
                    $outputPath = $uploadDir . $outputFilename;
...
<head>
    ...
    <title>Group Video Editor - <?= htmlspecialchars($group['name']) ?></title>
...
<body>
    ...
    <div class="processing-overlay" id="processingOverlay">
        ...
        <div class="processing-text" id="processingText">Processing your group video...</div>
    </div>
    ...
    <div class="editor-container">
        ...
        <div class="editor-header">
            ...
            <div class="header-actions">
                ...
                <button class="btn btn-secondary" onclick="window.location.href='group_post.php?group_id=<?= $groupId ?>'">
    ...
    <div class="modal" id="imageUploadModal">
        <div class="modal-content">
            <div class="modal-header">
                <h3>Create Group Video from Images</h3>
            ...
            <button class="btn btn-primary" style="width: 100%;" onclick="uploadMultipleImages()">
                <i class="fas fa-film"></i> Create Group Video
            </button>
        </div>
    </div>
    ...
    <div class="modal" id="audioModal">
        <div class="modal-content">
            <div class="modal-header">
                <h3>Add Audio to Group Video</h3>
    ...
    <div class="modal" id="textModal">
        <div class="modal-content">
            <div class="modal-header">
                <h3>Add Text to Group Video</h3>
                <button class="close-modal" onclick="closeModal('textModal')">&times;</button>
            </div>
            <div class="form-group">
                <label>Text Content:</label>
                <input type="text" class="text-input" id="textContent" placeholder="Enter your text here" value="<?= htmlspecialchars($group['name']) ?>">
            </div>
            <div class="form-group">
    ...
    <div class="modal" id="exportModal">
        <div class="modal-content">
            ...
            <button class="btn btn-primary" style="width: 100%;" onclick="exportVideo()">
                <i class="fas fa-download"></i> Export Group Video
            </button>
        </div>
    </div>
    ...
    <div class="modal" id="frameManagerModal">
        <div class="modal-content">
            <div class="modal-header">
                <h3>Manage Group Video Frames</h3>
    ...
    <div class="modal" id="filtersModal">
        <div class="modal-content">
            <div class="modal-header">
                <h3>Apply Filter to Group Video</h3>
    ...
    <div class="modal" id="duplicateFrameModal">
        <div class="modal-content">
            <div class="modal-header">
                <h3>Duplicate Frame in Group Video</h3>
    ...
    <div class="modal" id="speedModal">
        <div class="modal-content">
            <div class="modal-header">
                <h3>Adjust Group Video Speed</h3>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Check if we have a video to edit from session
            <?php if (isset($_SESSION['current_group_edit_video'])): ?>
                const editVideo = <?= json_encode($_SESSION['current_group_edit_video']) ?>;
                displayMediaPreview(editVideo.url, 'video', editVideo.frames || [], editVideo.duration || 0);
        ...
        function uploadMedia(file) {
            ...
            showProcessing('Uploading media for group...');
            ...
            .then(data => {
                hideProcessing();
                if (data.success) {
                    showToast('Media uploaded successfully for group!', 'success');
        ...
        function uploadMultipleImages() {
            ...
            showProcessing('Creating group video from images...');
            ...
            .then(data => {
                hideProcessing();
                if (data.success) {
                    showToast('Group video created successfully!', 'success');
                    ...
                    showToast('Failed to create group video: ' + (data.error || 'Unknown error'), 'error');
                }
            })
            .catch(error => {
                hideProcessing();
                console.error('Error:', error);
                showToast('Failed to create group video', 'error');
            });
        }
        ...
        function deleteSingleFrame(frameIndex) {
            ...
            showProcessing('Deleting frame and regenerating group video...');
            ...
            .then(data => {
                hideProcessing();
                if (data.success) {
                    showToast('Frame deleted and group video regenerated successfully!', 'success');
                    displayMediaPreview(data.url, 'video', data.frames, data.duration);
                    updateUndoRedoButtons();
                } else {
        ...
        function deleteSelectedFrames() {
            ...
            if (!confirm(`Are you sure you want to delete ${selectedFrames.size} frame(s)? This will regenerate the group video.`)) {
                return;
            }
            ...
            showProcessing(`Deleting ${frameIndices.length} frame(s) and regenerating group video...`);
            ...
            .then(data => {
                hideProcessing();
                if (data.success) {
                    showToast(`${frameIndices.length} frame(s) deleted and group video regenerated successfully!`, 'success');
                    displayMediaPreview(data.url, 'video', data.frames, data.duration);
                    selectedFrames.clear();
        ...
        function duplicateFrame() {
            ...
            showProcessing('Duplicating frame in group video...');
            ...
            .then(data => {
                hideProcessing();
                if (data.success) {
                    showToast('Frame duplicated successfully in group video!', 'success');
                    displayMediaPreview(data.url, 'video', data.frames, data.duration);
        ...
        function trimVideo() {
            ...
            showProcessing('Trimming group video...');
            ...
            .then(data => {
                hideProcessing();
                if (data.success) {
                    showToast('Group video trimmed successfully!', 'success');
                    displayMediaPreview(data.url, 'video', data.frames, data.duration);
                    closeModal('trimModal');
                    updateUndoRedoButtons();
                } else {
                    showToast('Failed to trim group video: ' + (data.error || 'Unknown error'), 'error');
                }
            })
            .catch(error => {
                hideProcessing();
                console.error('Error:', error);
                showToast('Failed to trim group video', 'error');
            });
        }
        ...
        function splitVideo() {
            ...
            showProcessing('Splitting group video...');
            ...
            .then(data => {
                hideProcessing();
                if (data.success) {
                    showToast('Group video split successfully!', 'success');
                    ...
                    showToast('Failed to split group video: ' + (data.error || 'Unknown error'), 'error');
                }
            })
            .catch(error => {
                hideProcessing();
                console.error('Error:', error);
                showToast('Failed to split group video', 'error');
            });
        }
        ...
        function adjustSpeed() {
            ...
            showProcessing('Adjusting group video speed...');
            ...
            .then(data => {
                hideProcessing();
                if (data.success) {
                    showToast('Group video speed adjusted successfully!', 'success');
                    displayMediaPreview(data.url, 'video', data.frames, data.duration);
        ...
        function addAudioToVideo() {
            ...
            showProcessing('Adding audio to group video...');
            ...
            .then(data => {
                hideProcessing();
                if (data.success) {
                    showToast('Audio added successfully to group video!', 'success');
                    displayMediaPreview(data.url, 'video', data.frames, data.duration);
        ...
        function createDraggableTextOverlay() {
            ...
            textOverlay.textContent = document.getElementById('textContent').value || '<?= htmlspecialchars($group['name']) ?>';
        ...
        function addTextToVideo() {
            ...
            showProcessing('Adding text to group video...');
            ...
            .then(data => {
                hideProcessing();
                if (data.success) {
                    showToast('Text added successfully to group video!', 'success');
                    displayMediaPreview(data.url, 'video', data.frames, data.duration);
        ...
        function removeAudio() {
            ...
            showProcessing('Removing audio from group video...');
            ...
            .then(data => {
                hideProcessing();
                if (data.success) {
                    showToast('Audio removed successfully from group video!', 'success');
                    displayMediaPreview(data.url, 'video', data.frames, data.duration);
                    updateUndoRedoButtons();
                } else {
        ...
        function exportVideo() {
            ...
            showProcessing('Exporting group video...');
            ...
            .then(data => {
                hideProcessing();
                if (data.success) {
                    showToast('Group video exported successfully!', 'success');
                    ...
                    showToast('Failed to export group video: ' + (data.error || 'Unknown error'), 'error');
                }
            })
            .catch(error => {
                hideProcessing();
                console.error('Error:', error);
                showToast('Failed to export group video', 'error');
            });
        }
        ...
        function applyFilter(filter) {
            ...
            showProcessing(`Applying ${filter} filter to group video...`);
            ...
            .then(data => {
                hideProcessing();
                if (data.success) {
                    showToast('Filter applied successfully to group video!', 'success');
                    displayMediaPreview(data.url, 'video', data.frames, data.duration);
                    updateUndoRedoButtons();
                } else {
        ...
        function selectTextColor(color) {
            ...
            document.querySelectorAll('.color-swatch').forEach(swatch => {
                swatch.style.borderColor = swatch.getAttribute('data-color') === color ? '#667eea' : '#3a3a4d';
            });
        }
        ...
        function confirmSaveAndRedirect() {
            ...
            modal.innerHTML = `
                <div class="modal-content">
                    <div class="modal-header">
                        <h3>Finish Group Video Editing</h3>
                        <button class="close-modal" onclick="this.parentElement.parentElement.parentElement.remove()">&times;</button>
                    </div>
                    <p>Are you sure you want to save this group video and proceed to sharing?</p>
                    <p><small>The group watermark will be applied automatically before saving.</small></p>
        ...
        function saveAndRedirect() {
            ...
            showProcessing('Applying group watermark and saving video...');
            ...
            .then(data => {
                hideProcessing();
                if (data.success) {
                    showToast('Group video saved successfully! Redirecting...', 'success');
                    setTimeout(() => {
                        window.location.href = data.redirect_url;
                    }, 1500);
                } else {
                    showToast('Failed to save group video: ' + (data.error || 'Unknown error'), 'error');
                }
            })
            .catch(error => {
                hideProcessing();
                console.error('Error:', error);
                showToast('Failed to save group video', 'error');
            });
        }
        ...
        function setupDragAndDrop() {
            ...
            function highlight() {
                dropArea.style.background = 'rgba(102, 126, 234, 0.3)';
            }
        ...
        function showProcessing(message = 'Processing your group video...') {
        ...
        function reverseVideo() {
            ...
            showProcessing('Reversing group video...');
            ...
            .then(data => {
                hideProcessing();
                if (data.success) {
                    showToast('Group video reversed successfully!', 'success');
                    displayMediaPreview(data.url, 'video', data.frames, data.duration);
                    updateUndoRedoButtons();
                } else {
                    showToast('Failed to reverse group video: ' + (data.error || 'Unknown error'), 'error');
                }
            })
            .catch(error => {
                hideProcessing();
                console.error('Error:', error);
                showToast('Failed to reverse group video', 'error');
            });
        }
        ...
        function extractAudio() {
            ...
            showProcessing('Extracting audio from group video...');
            ...
            .then(data => {
                hideProcessing();
                if (data.success) {
                    showToast('Audio extracted successfully from group video!', 'success');
        ...
        function stabilizeVideo() {
            ...
            showProcessing('Stabilizing group video...');
            ...
            .then(data => {
                hideProcessing();
                if (data.success) {
                    showToast('Group video stabilized successfully!', 'success');
                    displayMediaPreview(data.url, 'video', data.frames, data.duration);
                    updateUndoRedoButtons();
                } else {
                    showToast('Failed to stabilize group video: ' + (data.error || 'Unknown error'), 'error');
                }
            })
            .catch(error => {
                hideProcessing();
                console.error('Error:', error);
                showToast('Failed to stabilize group video', 'error');
            });
        }
