...
$viewedVideosStmt = $pdo->prepare("
    ...
    AND watch_time >= 2  -- Only count videos watched for at least 5 seconds
");
$viewedVideosStmt->execute([$userId]);
$viewedVideoIds = $viewedVideosStmt->fetchAll(PDO::FETCH_COLUMN, 0);

// NEW: Get user's skipped videos (watched less than 2 seconds)
$skippedVideosStmt = $pdo->prepare("
    SELECT DISTINCT post_id 
    FROM user_behavior_tracking 
    WHERE user_id = ? 
    AND interaction_type IN ('view', 'quick_skip')
    AND watch_time < 2  -- Videos watched less than 2 seconds are considered skipped
");
$skippedVideosStmt->execute([$userId]);
$skippedVideoIds = $skippedVideosStmt->fetchAll(PDO::FETCH_COLUMN, 0);

// NEW: Enhanced New User Onboarding System
class NewUserOnboarding {
    ...
    // Check if user is new (created within last 24 hours with less than 10 interactions)
    public function isNewUser() {
        try {
            $stmt = $this->pdo->prepare("
                ...
                FROM users u
                LEFT JOIN user_behavior_tracking ubt ON u.id = ubt.user_id
                WHERE u.id = ?
            ...
            if ($userData) {
                $userAge = time() - strtotime($userData['created_at']);
                $isNew = ($userAge < 86400 && $userData['interaction_count'] < 10);
                return $isNew;
            }
            return false;
        } catch (Exception $e) {
            error_log("New user check error: " . $e->getMessage());
            return false;
        }
    }
    
    // Get diverse category posts for new user exploration
    public function getDiverseCategoryPosts($limit = 20) {
        try {
            $categories = ['Entertainment', 'Music', 'Comedy', 'Dance', 'Gaming', 'Beauty and Fashion', 'Food and Cooking'];
            $allPosts = [];
            ...
            foreach ($categories as $category) {
                $stmt = $this->pdo->prepare("
                    ...
                    LIMIT ?
                ");
                $postsPerCategory = max(2, floor($limit / count($categories)));
                $stmt->execute([$category, $category, $category, $postsPerCategory]);
            ...
            error_log("Diverse posts error: " . $e->getMessage());
            return [];
        }
    }
    
    // Track user category preferences during onboarding
    public function trackOnboardingPreference($category, $interactionType) {
        try {
            $scoreWeights = [
                'view' => 0.1,
                'completion' => 0.3,
            ...
            $score = $scoreWeights[$interactionType] ?? 0.1;
            ...
            $stmt = $this->pdo->prepare("
                ...
                DO UPDATE SET 
                    preference_score = user_onboarding_preferences.preference_score + ?,
                    created_at = NOW()
            ");
            $stmt->execute([$this->userId, $category, $score, $score]);
            
            return true;
        } catch (Exception $e) {
            error_log("Onboarding preference error: " . $e->getMessage());
            return false;
        }
    }
    
    // Get user's preferred categories from onboarding
    public function getOnboardingPreferences() {
        try {
            $stmt = $this->pdo->prepare("
            ...
            error_log("Get onboarding prefs error: " . $e->getMessage());
            return [];
        }
    }
}

// NEW: Advanced Trending System with Time Decay
class AdvancedTrendingSystem {
    private $pdo;
    ...
    public function calculateTrendingScore($post, $timeWindow = '24 hours') {
        try {
            $baseScore = (
                ...
                ($post['avg_completion_rate'] ?? 0.5) * 5.0
            );
            
            // Apply time decay
            $hoursAgo = $post['hours_ago'] ?? 24;
            $timeDecay = $this->calculateTimeDecay($hoursAgo);
            
            // Engagement velocity bonus
            $engagementVelocity = $this->calculateEngagementVelocity($post['id'], $timeWindow);
            
            $trendingScore = $baseScore * $timeDecay * (1 + $engagementVelocity);
            
            return min($trendingScore, 1000); // Cap at 1000
        } catch (Exception $e) {
            error_log("Trending score calculation error: " . $e->getMessage());
            return 0;
        }
    }
    
    // Calculate time decay factor (exponential decay)
    private function calculateTimeDecay($hoursAgo) {
        $halfLife = 12; // Score halves every 12 hours
        return exp(-($hoursAgo / $halfLife) * log(2));
    }
    
    // Calculate engagement velocity (engagement per hour)
    private function calculateEngagementVelocity($postId, $timeWindow) {
        $stmt = $this->pdo->prepare("
            SELECT 
                COUNT(*) as engagements,
                EXTRACT(EPOCH FROM (NOW() - MIN(created_at))) / 3600 as hours_passed
            FROM (
        ...
        if ($data['hours_passed'] > 0) {
            return $data['engagements'] / $data['hours_passed'];
        }
        return 0;
    }
    
    // Get trending posts with enhanced criteria
    public function getTrendingPosts($limit = 20, $minEngagement = 50) {
        try {
            $stmt = $this->pdo->prepare("
                SELECT 
                    p.*,
                ...
                LEFT JOIN (
                    SELECT post_id, AVG(completion_rate) as avg_completion_rate
                    FROM video_views 
                ...
                AND (COALESCE(l.likes_count, 0) + COALESCE(c.comments_count, 0) * 2 + COALESCE(s.shares_count, 0) * 3) >= ?
                ORDER BY (COALESCE(l.likes_count, 0) + COALESCE(c.comments_count, 0) * 2 + COALESCE(s.shares_count, 0) * 3) DESC
                LIMIT ?
            ");
            $stmt->execute([$minEngagement, $limit]);
...
// NEW: Post Sequencing and Display Manager
class PostDisplayManager {
    ...
    // Create optimized post sequence with trending post spacing
    public function createOptimizedSequence($posts, $trendingPosts, $minGapBetweenTrending = 3) {
        $regularPosts = [];
        $trendingPostsMap = [];
        
        // Separate regular and trending posts
        $trendingIds = array_column($trendingPosts, 'id');
        foreach ($posts as $post) {
            if (in_array($post['id'], $trendingIds)) {
                $trendingPostsMap[$post['id']] = $post;
            } else {
                $regularPosts[] = $post;
            }
        }
        ...
        $trendingCount = 0;
        $regularCount = 0;
        $lastTrendingPosition = -$minGapBetweenTrending; // Ensure first trending can be placed early
        
        // Create sequence with proper spacing
        while (count($sequence) < count($posts) && (count($regularPosts) > 0 || count($trendingPostsMap) > 0)) {
            $currentPosition = count($sequence);
            
            // Check if we can place a trending post
            $canPlaceTrending = ($currentPosition - $lastTrendingPosition) >= $minGapBetweenTrending && 
                               count($trendingPostsMap) > 0;
            
            if ($canPlaceTrending) {
                // Get next trending post
                $trendingPost = array_shift($trendingPostsMap);
                $sequence[] = $trendingPost;
                $lastTrendingPosition = $currentPosition;
                $trendingCount++;
            } else if (count($regularPosts) > 0) {
                // Place regular post
                $sequence[] = array_shift($regularPosts);
                $regularCount++;
            } else {
                // If no regular posts left but trending posts remain, place them with minimum gap
                $trendingPost = array_shift($trendingPostsMap);
                $sequence[] = $trendingPost;
                $lastTrendingPosition = $currentPosition;
                $trendingCount++;
        ...
        return [
            'sequence' => $sequence,
            'stats' => [
                'total_posts' => count($sequence),
                'trending_posts' => $trendingCount,
                'regular_posts' => $regularCount,
                'trending_ratio' => $trendingCount / max(count($sequence), 1)
            ]
        ];
    }
    
    // Insert new user posts every 5 posts based on category matching
    public function insertNewUserPosts($sequence, $newUserPosts, $insertInterval = 5) {
        if (empty($newUserPosts)) {
            return $sequence;
        }
        ...
        foreach ($sequence as $index => $post) {
            $enhancedSequence[] = $post;
            
            // Insert new user post every specified interval
            if (($index + 1) % $insertInterval === 0 && $newUserPostIndex < count($newUserPosts)) {
                $newUserPost = $newUserPosts[$newUserPostIndex];
                $newUserPost['is_new_user_content'] = true;
                $newUserPost['insertion_reason'] = 'New user content discovery';
                $enhancedSequence[] = $newUserPost;
                $newUserPostIndex++;
            }
        }
...
// NEW: Enhanced User Preference Tracking with Skip Tracking
class UserPreferenceTracker {
    ...
    // NEW: Enhanced skip tracking - track when user watches less than 2 seconds
    public function trackVideoSkip($postId, $authorId, $watchTime = 0) {
        try {
            // Strong negative signal for skipping
            $skipStrength = -1.5;
            ...
            // Also track category negative preference
            $this->updateCategoryPreferenceFromPost($postId, -0.8);
            
            // Track hashtag negative preferences
            $this->updateHashtagPreferences($postId, -0.8);
            
            // Log the skip for analytics
            error_log("User {$this->userId} skipped video {$postId} from author {$authorId} after {$watchTime} seconds");
    ...
    // NEW: Get author skip rate (percentage of videos skipped)
    public function getAuthorSkipRate($authorId) {
        try {
            $stmt = $this->pdo->prepare("
                SELECT 
                    COUNT(CASE WHEN signal_type = 'video_skip' THEN 1 END) as skip_count,
            ...
            if ($result['total_interactions'] > 0) {
                return $result['skip_count'] / $result['total_interactions'];
            }
            return 0;
        } catch (Exception $e) {
            error_log("Author skip rate error: " . $e->getMessage());
            return 0;
        }
    }
...
class CountryContentPrioritizer {
   ...
   private function getCountryRegions() {
    return [
        ...
        // Middle East countries are included in Asia above
        // Additional regional groupings for better content matching
...
// Enhanced TikTok Algorithm Class with NEW features including skip tracking
class TikTokAdvancedAlgorithm {
    ...
    // NEW: Enhanced preference-based scoring with skip penalty
    public function calculateUserPreferenceScore($post) {
        ...
        // NEW: Apply skip rate penalty
        $authorSkipRate = $this->preferenceTracker->getAuthorSkipRate($authorId);
        $skipPenalty = $authorSkipRate * 10; // Reduce score based on skip rate
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // USER BEHAVIOR TRACKING HANDLER - MODIFIED: Track skips (less than 2 seconds)
    if (isset($_POST['action']) && $_POST['action'] === 'track_behavior' && isset($_POST['post_id'])) {
        ...
        try {
            ...
            if ($interactionType === 'completion') {
                ...
                // NEW: Track video skip (less than 2 seconds watch time)
...
$onboardingSystem = new NewUserOnboarding($pdo, $userId);
$isNewUser = $onboardingSystem->isNewUser();

// NEW: Initialize trending system
$trendingSystem = new AdvancedTrendingSystem($pdo);
$trendingPosts = $trendingSystem->getTrendingPosts(15);

// NEW: Initialize post display manager
$postDisplayManager = new PostDisplayManager($pdo, $userId);
...
// NEW: Also exclude skipped videos
$excludeSkippedCondition = "";
if (!empty($skippedVideoIds)) {
    $placeholders = implode(',', array_fill(0, count($skippedVideoIds), '?'));
    $excludeSkippedCondition = "AND p.id NOT IN ($placeholders)";
    $params = array_merge($params, $skippedVideoIds);
}
...
// NEW: For new users, show diverse content first
if ($isNewUser) {
    $diversePosts = $onboardingSystem->getDiverseCategoryPosts(15);
    if (!empty($diversePosts)) {
        // Mark these posts as onboarding content
        foreach ($diversePosts as &$diversePost) {
            $diversePost['is_onboarding_content'] = true;
            $diversePost['tiktok_score'] = 75; // Give them a good score to ensure visibility
        }
        // Prepend diverse posts to the beginning
        $scoredPosts = array_merge($diversePosts, $scoredPosts);
    }
}

// NEW: Implement optimized post sequencing with trending post spacing
$sequenceResult = $postDisplayManager->createOptimizedSequence($scoredPosts, $trendingPosts, 3);
$finalPosts = $sequenceResult['sequence'];

// NEW: Insert new user posts every 5 posts for content discovery
$newUserPosts = array_filter($scoredPosts, function($post) {
    return $post['views_count'] < 50 && $post['likes_count'] < 10; // New content criteria
});
$finalPosts = $postDisplayManager->insertNewUserPosts($finalPosts, array_slice($newUserPosts, 0, 10), 5);
...
/* NEW: Onboarding and Discovery Indicators */
...
<div class="algorithm-dashboard">
    <div class="algorithm-stats">
        ...
        <div class="stat-item">
            <div class="stat-value"><?= count($skippedVideoIds) ?></div>
            <div class="stat-label">Skipped Videos</div>
        </div>
        <div class="stat-item">
...
<!-- NEW: Onboarding Message for New Users -->
<?php if ($isNewUser): ?>
<div class="algorithm-dashboard" style="background: linear-gradient(135deg, #00bcd4 0%, #009688 100%);">
    <h3 style="margin: 0 0 15px 0; text-align: center;">🎉 Welcome New User!</h3>
    <p style="text-align: center; margin: 0; font-size: 14px;">
        We're showing you diverse content to learn your preferences. 
        Watch videos you like to help us personalize your feed!
    </p>
</div>
<?php endif; ?>
        ...
        <?php foreach ($posts as $post): ?>
            <div class="reel" data-post-id="<?= $post['id'] ?>">
                ...
                <div class="advanced-metrics">
                    ...
                    <?php endif; ?>
                    <?php if (isset($post['is_onboarding_content']) && $post['is_onboarding_content']): ?>
                        <div class="metric-indicator onboarding-indicator">
                            🎯 Onboarding
                        </div>
                    <?php endif; ?>
                    <?php if (isset($post['is_new_user_content']) && $post['is_new_user_content']): ?>
                        <div class="metric-indicator discovery-indicator">
                            🔍 Discovery
                        </div>
                    <?php endif; ?>
                </div>
...
<script>
// Enhanced User Behavior Tracking with Skip Detection
class AdvancedBehaviorTracker {
    ...
    initAdvancedTracking() {
        ...
        this.initSkipDetection();
    }
    
    initSkipDetection() {
        // Track when user quickly scrolls past videos (potential skips)
        let videoVisibilityTime = new Map();
        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                ...
                if (entry.isIntersecting) {
                    ...
                    videoVisibilityTime.set(postId, Date.now());
                } else {
                    // Video became invisible - check if it was a quick skip
                    const visibleTime = videoVisibilityTime.get(postId);
                    if (visibleTime) {
                        const timeVisible = Date.now() - visibleTime;
                        if (timeVisible < 2000) { // Less than 2 seconds
                            this.trackPotentialSkip(postId, timeVisible);
                        }
                        videoVisibilityTime.delete(postId);
                    }
                }
            });
        }, { threshold: 0.5 });
    ...
    trackPotentialSkip(postId, visibleTime) {
        // Only track if user didn't actually watch the video
        if (!this.watchData.has(postId) || this.watchData.get(postId).totalWatchTime < 2) {
            const formData = new FormData();
            formData.append('action', 'track_behavior');
            formData.append('post_id', postId);
            formData.append('watch_time', visibleTime / 1000);
            formData.append('completion_rate', 0);
            formData.append('scroll_velocity', 0);
            formData.append('interaction_type', 'quick_skip');
            ...
            fetch('video.php', {
              ...
              .then(data => {
                  if (data.success) {
                      this.showLearningIndicator('📝 Noted your quick skip');
                  }
              })
              .catch(error => console.error('Skip tracking error:', error));
        }
    }
    ...
    trackVideoWatch(postId, videoIndex, currentTime, duration, eventType) {
        ...
        if (eventType === 'play') {
            ...
            postData.totalWatchTime = Math.max(postData.totalWatchTime, currentTime);
            ...
            // Track quick scroll away (less than 2 seconds) - SKIP DETECTION
   ...
   btn.addEventListener('click', async () => {
       ...
       try {
           ...
           if (data.success) {
               ...
               if (engagementSpan) {
                   const currentViews = parseInt(btn.getAttribute('data-views') || '1');
...
// Video Reel functionality with UNMUTED videos by default
document.querySelectorAll('.video-reel-container').forEach(container => {
    ...
    // Initialize first video - UNMUTED by default
    if (videos.length > 0) {
    ...
    container.querySelectorAll('.sound-toggle').forEach(button => {
        // Initialize button state - videos are UNMUTED by default
...
// Enhanced video click handling for the entire document
document.addEventListener('click', function(e) {
    // If click is on a video, handle play/pause
...
// Prevent video controls from triggering play/pause when clicking controls
...
document.addEventListener('DOMContentLoaded', function() {
    const allVideos = document.querySelectorAll('video');
    allVideos.forEach(video => {
        video.muted = false; // Ensure all videos are unmuted
    });
});
